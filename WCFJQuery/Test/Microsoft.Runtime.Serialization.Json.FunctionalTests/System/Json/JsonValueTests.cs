namespace System.Json.Test
{
    ...
    public class JsonValueTests
    {
        ...
        /// <summary>
        /// Test for casting JSON collections into CLR collection types.
        /// </summary>
        [TestMethod]
        public void CastToCollections()
        {
            JsonArray ja = JsonValue.Parse("[1, 3, 5]") as JsonArray;
            JsonObject jo1 = JsonObject.Parse("{\"A\":1,\"B\":2}") as JsonObject;
            JsonObject jo2 = JsonValue.Parse("{\"A\":1,\"B\":[11, {\"B1\":34.5,\"B2\":[55,66,77],\"B3\":35.4}, 33],\"C\":3, \"D\":{\"DD\":{\"DDD\":20}}}") as JsonObject;

            IList<object> list = (List<object>)ja;

            for (int i = 0; i < ja.Count; i++)
            {
                Assert.AreEqual((int)ja[i], (int)list[i]);
            }

            IDictionary<string, object> dict1 = (Dictionary<string, object>)jo1;
            Assert.AreEqual((int)jo1["A"], (int)dict1["A"]);
            Assert.AreEqual((int)jo1["B"], (int)dict1["B"]);

            IDictionary<string, object> dict2 = (Dictionary<string, object>)jo2;

            dynamic dyn2 = jo2;
            List<object> listItem;
            Dictionary<string, object> dictItem;

            listItem = dict2["B"] as List<object>;
            Assert.IsNotNull(listItem);

            dictItem = listItem[1] as Dictionary<string, object>;
            Assert.IsNotNull(dictItem);

            listItem = dictItem["B2"] as List<object>;
            Assert.IsNotNull(listItem);

            Assert.AreEqual((int)dyn2.B[1].B2[2], (int)listItem[2]);
        }

        /// <summary>
        /// Test that the types of collections in objects obtains from casting JsonValue to
        /// CLR collections are consistent across all children in the parent-child hierarchy.
        /// </summary>
        [TestMethod]
        public void CastToCollectionsTypeTest()
        {
            JsonArray ja = JsonValue.Parse("[1, {\"A\":[1,2,3]}, 5]") as JsonArray;
            JsonObject jo1 = JsonValue.Parse("{\"A\":1,\"B\":[1,2,3]}") as JsonObject;

            IList<object> list = (List<object>)ja;
            Array array = (Array)ja;
            object[] objArray = (object[])ja;
            object coll;

            coll = ((IDictionary<string, object>)list[1])["A"];
            Assert.AreEqual(typeof(List<object>), coll.GetType());

            coll = ((IDictionary<string, object>)array.GetValue(1))["A"];
            Assert.AreEqual(typeof(object[]), coll.GetType());

            coll = ((IDictionary<string, object>)objArray[1])["A"];
            Assert.AreEqual(typeof(object[]), coll.GetType());

            IDictionary<string, object> dict1 = (Dictionary<string, object>)jo1;
            Assert.AreEqual(typeof(List<object>), dict1["B"].GetType());
        }
