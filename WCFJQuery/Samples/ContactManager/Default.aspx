...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        <ul id="contacts">
            <asp:SqlDataSource ID="ContactManager" runat="server" 
                ConnectionString="<%$ ConnectionStrings:ContactManagerConnectionString %>" 
                SelectCommand="SELECT * FROM [Contact] ORDER BY [Name]">
                </asp:SqlDataSource>
            <asp:Repeater ID="Repeater1" runat="server" DataSourceID="ContactManager">
                <ItemTemplate>
                    <li>
                        <h1><%#DataBinder.Eval(Container.DataItem, "Name")%></h1>
                        <p><%#DataBinder.Eval(Container.DataItem, "Address")%>, <%#DataBinder.Eval(Container.DataItem, "City")%> <%#DataBinder.Eval(Container.DataItem, "State")%> <%#DataBinder.Eval(Container.DataItem, "Zip")%><br />
                            <a href="mailto:<%#DataBinder.Eval(Container.DataItem, "Email")%>"><%#DataBinder.Eval(Container.DataItem, "Email")%></a><br />
                            <a href="http://twitter.com/<%#DataBinder.Eval(Container.DataItem, "Twitter")%>">@<%#DataBinder.Eval(Container.DataItem, "Twitter")%></a></p>
                        <p><a href="contacts/<%#DataBinder.Eval(Container.DataItem, "ContactId")%>" class="removeContact">Remove</a></p>
                    </li>
                </ItemTemplate>
            </asp:Repeater>
        </ul>
